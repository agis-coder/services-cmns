...
export class MailService {
    ...
    constructor() {
        this.transporter = nodemailer.createTransport({
            ...
            secure: false,
            auth: {
                user: 'thongtinduanhot2026@gmail.com',
                pass: 'igti zszf eoys weno',
            },
        })
    }

    async sendBulkMail(dto: SendBulkMailDto) {
        const { to = [], cc = [], bcc = [], subject, htmlContent, textContent, attachments, replyTo, fromName, fromEmail, } = dto
        if (!subject?.trim()) {
        ...
        const normalizeEmails = (list: string[]) => [...new Set(list.map(e => e.trim().toLowerCase()))]
        ...
        if (invalidEmails.length) {
            throw new BadRequestException(`Email không hợp lệ: ${invalidEmails.join(', ')}`,)
        }

        const info = await this.transporter.sendMail({
            from: fromEmail ? `"${fromName || ''}" <${fromEmail}>` : `"THÔNG TIN DỰ ÁN HOT" <${process.env.MAIL_USER}>`,
